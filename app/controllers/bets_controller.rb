class BetsController < ApplicationController
	def create
		...
		logger.info ":bet:monto = #{params[:bet][:monto].to_i}" 
		if @user.pezzos >= params[:bet][:monto].to_i
	
			@bet= @partido.bets.new(params[:bet])
			if !@bet.save
				redirect_to partido_path(@partido, notice: "No se pudo crear la apuesta! Te devolvimos el dinero")
				return
			end
			...
			#gamification
			Experience.new to: @user, from: :bet
